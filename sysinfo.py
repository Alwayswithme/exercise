...
class Disk(Info):
    ...
    def __str__(self):
        spaces = Info.spaces
        info = []
        disk_detail = [ self.extract_disk_detail(i) for i in self.details]
        for i in disk_detail:
            if i:
                info.append('{0}{1}\n'.format(spaces, i))
    ...
    def extract_disk_detail(self, disk):
        line = '{node}: {device} {capacity}'.format(node=disk['node'], device=disk['device'],
...
class Memory(Info):
    ...
    def __str__(self):
        ...
        mem_detail = [i for i in mem_detail if i]
        for i in mem_detail:
            if i:
                info.append('{0}{1}\n'.format(spaces, i))
        info[-1] = info[-1].replace('│', '└')
